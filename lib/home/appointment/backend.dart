...
class AppointmentBackend {
  Future<void> upcomingToCompleted(
    ...
    // Ensure the upcoming list is not empty before processing
    while (authNotifier.patientDetails!.upcoming!.isNotEmpty &&
        authNotifier.patientDetails!.upcoming![0].dateTime!
            .isBefore(DateTime.now())) {
      // Check if the upcoming list is not empty before accessing
      if (DoctorsUpcoming.isNotEmpty &&
          authNotifier.patientDetails!.upcoming!.isNotEmpty) {
  ...
  Future<void> upcomingToCompletedbyDoctor(
    ...
    if (authNotifier.doctorDetails!.upcoming!.isNotEmpty &&
      ...
      // Ensure the recursion stops when the lists are empty
      if (authNotifier.doctorDetails!.upcoming!.isNotEmpty) {
  ...
  Future<void> cancelAppointment(
    ...
    // Check if the upcoming list has enough elements
    if (authNotifier.patientDetails!.upcoming != null &&
        authNotifier.patientDetails!.upcoming!.isNotEmpty &&
        index < DoctorsUpcoming.length) {
      ...
      DoctorsCancelled[0].upcoming!.removeWhere((object) =>
          object.dateTime ==
          authNotifier.patientDetails!.cancelled![0].dateTime);
      await FirebaseFirestore.instance
          .collection('Doctors')
